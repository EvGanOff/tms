...
class ViewController: UIViewController, ViewControllerResultDelegate {
    ...
    func result(_ textField1: Int, textField2: String?) {
        let color1 = UIColor(red: 150 / 255, green: 80 / 255, blue: 50 / 255, alpha: 1).cgColor
        let color2 = UIColor(red: 210 / 255, green: 20 / 255, blue: 61 / 255, alpha: 1).cgColor
        let gradietLayer1 = CAGradientLayer()
        gradietLayer1.frame = view.bounds
        gradietLayer1.colors = [color1, color2]
        gradietLayer1.startPoint = CGPoint(x: 0, y: 0)
        gradietLayer1.endPoint = CGPoint(x: 0, y: 1.0)
        let color3 = UIColor(red: 150 / 255, green: 150 / 255, blue: 50 / 255, alpha: 1).cgColor
        let color4 = UIColor(red: 210 / 255, green: 150 / 255, blue: 61 / 255, alpha: 1).cgColor
        let gradietLayer2 = CAGradientLayer()
        gradietLayer2.frame = view.bounds
        gradietLayer2.colors = [color3, color4]
        gradietLayer2.startPoint = CGPoint(x: 0, y: 0)
        gradietLayer2.endPoint = CGPoint(x: 0, y: 1.0)
        if (textField1 > 50) && textField2 == "M" {
            self.view.layer.insertSublayer(gradietLayer1, at: 1)
        }
        if (textField1 < 50) && textField2 == "M" {
            return
        }
        if  textField2 == "F" {
            self.view.layer.insertSublayer(gradietLayer2, at: 2)
        }
